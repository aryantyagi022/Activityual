<script lang="ts">
  ...
  let email = '', password = '', displayName = '', submitting = false;
...
<div class="max-w-sm mx-auto bg-white p-6 rounded shadow mt-10">
  ...
  <form on:submit|preventDefault={submit} class="space-y-3">
    <input class="w-full border rounded px-3 py-2" placeholder="Display name" bind:value={displayName} required />
    <input class="w-full border rounded px-3 py-2" type="email" placeholder="Email" bind:value={email} required />
    <input class="w-full border rounded px-3 py-2" type="password" placeholder="Password (min 8)" bind:value={password} required minlength="8" />
    <button class="w-full bg-indigo-600 text-white py-2 rounded disabled:opacity-50" disabled={submitting}>{submitting ? 'Creating…' : 'Register'}</button>
  </form>
</div>
